...
# Стрелочные функции
В js можно писать функции проще

```js
function long_add(a, b) {
    return a + b;
}
const short_add = (a, b) => a + b;

console.log(long_add(3,5));
console.log(short_add(3, 5));
```

>При записи функций в переменные лучше всегда использовать const, чтобы мы не могли случайно ее изменить

Функции делают одно и то же. Название  берется из имени переменной, аргументы - из скобок. return не используется - вернется результат из строки (`a + b` в нашем случае)

Если очень хочется можно написать стрелочную функцию больше чем в 1 строку, но тогда понадобятся фигурные скобки и return

```js
const middle_size_add = (a, b) => {
	return a + b;
}
```
